...
output=$(curl -s "http://${service_map[secure-python-app]}/helper.php?fetch=http://sodiumlaurethsulfatedesyroyer.com" \
  --insecure)
if [ $? -eq 0 ]; then
...
if echo "$output" | grep -q "sodiumlaurethsulfatedesyroyer.com"; then
